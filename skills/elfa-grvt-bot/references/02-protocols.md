...
Wire format (captured 2026-05-13 against `api.elfa.ai`, see `captured-frames/notification_cron_once_2026-05-13.txt` and `notification_price_current_2026-05-13.txt`):

```
id: 1b52d892-ba80-42d8-b421-36586410f23b
event: notification
data: {"status":"triggered","title":"Auto Plan Alert","body":"...","message":"...","queryId":"<uuid>","queryTitle":"...","autoDetails":"...","executionId":"<uuid>","triggerTime":"2026-05-13T06:53:25.405Z","queryIdShort":"...","conditionsMet":1,"queryDisplayTitle":"...","timestamp":1778655205405}

```

Keep-alive comments arrive every ~15s:
...
**Required fields the parser MUST check (drop with WARN if any missing):**

- `status` == `"triggered"` (string)
- `queryId` matches the stream URL's query id (string)
- `executionId` non-empty string (UUID)
- `triggerTime` non-empty string (ISO 8601)

**Informational fields the parser passes through unchanged:**

- `timestamp` (epoch ms)
- `title`, `body`, `message` (human strings)
- `queryTitle`, `queryIdShort`, `queryDisplayTitle`, `autoDetails`
- `conditionsMet` (integer count)

**Documented canonical-envelope fields the parser MUST NOT require:** `version`, `eventType`, `eventId`, `channel`, `trigger`, `evaluation`, `action`. These appear in `docs.elfa.ai/auto/notifications` but are NOT emitted by production today. Treating them as required drops every real fire.

**Forward compatibility:** accept both `event: notification` (production today) and `event: query.triggered` (if Elfa rolls out the documented envelope later). Tolerate new fields. Adding new required fields is the breaking case; capture a fresh frame and update this spec.
...
**Live caveat:** an HTTP 200 or syntactically accepted `bulk_orders` response is not enough to call the trade placed. A live test returned an accepted-looking response while the UI showed parent `Rejected` and TP/SL `Cancelled`. After any bulk submit, immediately verify the parent by `client_order_id` via open orders / order history / positions. Emit `grvt_other` and do not emit `order_placed` unless the parent is accepted or filled. Keep the strategy single-fire either way.
...
3. Extract the signed Order object (`signed_order = payload["order"]` if the SDK returns a wrapper, else `payload`).
4. **After signing**, inject `signed_order["metadata"]["trigger"] = {...}`.
5. Add the signed Order object to the `orders` array. Do not add the wrapper dict itself.
...
### Size, lot step, and min notional

GRVT instrument metadata includes string fields like:
...
Use `Decimal` and enforce both before strategy creation and again at fire time:

1. `amount >= min_size`
2. `amount` is an integer multiple of `min_size` (unless a separate size step is exposed)
3. `amount * reference_price >= min_notional`
4. `amount * reference_price <= max_notional_usd`

Do not choose smoke-test amounts by notional alone. Example observed: BTC `0.00126` was above 100 USDT but rejected as `Order size too granular`; BTC `0.002` is the smallest valid step above min notional near an 80k price.
